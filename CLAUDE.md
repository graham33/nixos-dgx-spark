...
- Pull requests from forks get the `check` job only. GitHub withholds every secret -- repository, organisation and environment alike -- from a `pull_request` run whose head is another repository, so `secrets.NIXBUILD_TOKEN` is empty and no label or approval gate can hand one back; `build` and `full-build` skip rather than fail. To build one, dispatch CI manually with `pr` set to the PR number (add `full_build` for the expensive job): `gh workflow run ci.yml -f pr=173 [-f full_build=true]`. That runs in this repository's context, so the token is in scope -- which also means it runs the contributor's code with the token, so read the diff first. Dispatching needs write access, which is what keeps contributors from starting these builds themselves.
