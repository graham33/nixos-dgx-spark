...
in
{
  ...
  config = mkIf cfg.enable {
    # Apply overlay to allow using linux_6_17 despite EOL status
    # This is necessary because we use NVIDIA's maintained 6.17 branch
    nixpkgs.overlays = [ (import ../overlays/linux-6.17.nix) ];

    # Use the NVIDIA kernel if enabled, otherwise use standard 6.17 kernel
